...
## Truncated cumulative incidence function
...
## Function to invert the CIF and sample from it. 
...
## Function to simulate times and events
...
## Function to draw random parameters from the posterior distribution of a brms fit
...
## Function compiling empty brms model
compile.analysis.model <- function(dataset) {
  ...
  formula <- bf(time.predicted | cens(censored1) ~   0 + Intercept + TREATMENT + c,
                family = weibullPH)
...
main.analysis.update <- function(dataset, fit.null){ ## event considered is contained in EVENT.PREDICTED
  ## starting from main dataset and formula to analyze data 
  ...
  ## and probability of recovery graduating : p(HR >1 | Data) = # draws with HR >1 / 4000
...
fit.noPH <- function(dataset, 
                     brms_function1, 
                     brms_function2){  ### models fitted within treatment arm
...
predicted_dataset_noPH <- function (data_to_predict, data_observed, n.sim, 
                                    model1, model2 , cores){
  ...
  predicted <- mclapply( 1:n.sim, 
                         function(x) {
                           predicted <- rand_crisk_w(
                             ...
                             model.matrix(~ c, data = data_to_predict[[x]]), ### checkkk!!!!
                             data_to_predict[[x]]$TIME ### time of real censoring
                           )
...
predicted_dataset_PH.HR <- function (data_to_predict, data_observed, n.sim, 
  ...
  predicted <- mclapply( 1:n.sim, 
                         function(x) {
                           predicted <- rand_crisk_w(
                             ...
                             as.numeric(r.beta.HR$fit.1[x,]),
                             as.numeric(r.beta.HR$fit.2[x,]),
                             model.matrix(~ TREATMENT + c, data = data_to_predict[[x]]), ### checkkk!!!!
                             data_to_predict[[x]]$TIME ### dat_inv0$maxFU18??? or time of real censoring
                           )
